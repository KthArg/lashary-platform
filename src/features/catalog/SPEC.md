...
historias:
  ...
  - id: US-PROD-01
    estado: en_progreso
    falta: "correr contra Supabase local los tests package-repository.integration.test.ts y package-rls-isolation.test.ts, y abrir /admin/catalog/packages en un navegador para probar el formulario a mano; el codigo esta completo (dominio, application, db, RLS, server actions, UI, contrato) y sus pruebas unitarias corren en verde, pero sin esa ejecucion no hay evidencia para pasar a terminada (EST-005)"
  - id: US-PROM-01
    estado: no_iniciada
  - id: US-PROM-02
    estado: no_iniciada
...
Dónde se detiene US-PROD-01: el código está completo y sus pruebas unitarias (dominio, application con repositorios en memoria, schema y server actions con repositorios mockeados) corren en verde. Lo que **no** se ejecutó: `db/__tests__/package-repository.integration.test.ts` y `__tests__/package-rls-isolation.test.ts` contra una base real, y la UI en un navegador — el entorno donde se escribió no tiene Docker y su `.env.local` apunta a producción, que no se tocó. Sin esa ejecución no hay evidencia (EST-005), así que la historia queda `en_progreso`, no `terminada`.
