...
### scheduling (actualizado: 2026-09-16)
- US-AGE-01 — en_progreso — falta: Bloqueo manual puntual (scheduling_manual_blocks) — próximo incremento de la misma historia; panel administrativo (UI) y su wiring en src/app; el criterio 'reducir disponibilidad no elimina citas ya agendadas' queda diferido — depende de scheduling_appointments, que no existe hasta US-AGE-05
...
- scheduling: Test de aislamiento RLS (scheduling_weekly_availability, scheduling_closed_dates) contra instancia local de Supabase en CI — mismo patrón aceptado en auth (PR #3) — aceptada en PR US-AGE-01 (disponibilidad semanal) — costo: 2h
