...
> **Estado:** aceptado. **Fecha:** 2026-08-27. **Decisores:** PO (decisión), bootstrap (objeción registrada). **Historias:** US-AGE-05, US-AGE-09. **Reglas:** DOM-010, DOM-012.

## Contexto

El texto original de US-AGE-09 pedía aceptar o rechazar reservas manualmente; sus criterios de aceptación describían el constraint de no-solape. Son dos entregables de la misma historia: US-AGE-09 entrega el constraint (ADR-0002) Y el flujo de aprobación (el backlog no admite historias nuevas, ADR-0007). El PO eligió explícitamente el flujo con aprobación manual (opción 8b del cuestionario de bootstrap, 2026-08-27).
...
- **Rechazar** → libera el espacio inmediatamente, notifica a la clienta con motivo opcional, y queda en la bitácora (portada por US-AGE-13, ADR-0007).
